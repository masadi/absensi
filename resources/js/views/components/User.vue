        ...
        <div class="col-md-4 offset-md-4">
      	...
      	<!-- BLOCK INI AKAN MENGHASILKAN LIST DATA DALAM BENTUK TABLE MENGGUNAKAN COMPONENT TABLE DARI BOOTSTRAP VUE -->
            ...
            <b-table :striped="true" :bordered="true" hover :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" show-empty>
                <template v-slot:cell(bentuk_pendidikan_id)="row">
                    {{(row.item.bentuk_pendidikan_id == 5) ? 'SD' : 'SMP'}}
                </template>
                <template v-slot:cell(actions)="row">
                    ...
                    <button class="btn btn-warning btn-sm" @click="cetakKartu(row.item)">Cetak Kartu Registrasi</button>
                    <button class="btn btn-primary btn-sm" @click="resetPassword(row.item)">Reset Password</button>
...
export default {
    ...
    methods: {
        ...
        cetakKartu(item){
            window.open('/cetak/akun/'+item.user_id, '_blank')
            console.log(item);
        },
        resetPassword(item){
            console.log(item);

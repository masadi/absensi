    ...
    <div class="auth-inner my-2">
      ...
      <div class="card mb-0">
        <div class="card-body">
          ...
          <h4 class="card-title mb-1">Selamat Datang di {{ config('app.name', 'Laravel') }} 🚀</h4>
          <p class="card-text mb-2">Silahkan mengisi formulir berikut untuk registrasi</p>


          <form class="auth-register-form mt-2" method="POST" action="{{ route('register') }}">
            @csrf
            <div class="mb-1">
              <label for="register-username" class="form-label">Nama Lengkap (tanpa gelar)</label>
              <input type="text" class="form-control @error('name') is-invalid @enderror" id="register-username"
                name="name" placeholder="Nama Lengkap" aria-describedby="register-username" tabindex="1" autofocus
                value="{{ old('name') }}" />
              @error('name')
            ...
            <div class="mb-1">
              <label for="register-email" class="form-label">Email Aktif</label>
              <input type="text" class="form-control @error('email') is-invalid @enderror" id="register-email"
                name="email" placeholder="contoh@email.com" aria-describedby="register-email" tabindex="2"
                value="{{ old('email') }}" />
              @error('email')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
